...
//
#DECLARE($folder_platformPath : Text)
// ----------------------------------------------------
...
If (DEV_ASSERT_PARMCOUNT(Current method name:C684; 1; Count parameters:C259))
	...
	If (Folder_DoesExist($folder_platformPath))
		$folder_platformPath:=$folder_platformPath+Folder separator:K24:12  // ready for our code below
		...
		DOCUMENT LIST:C474($folder_platformPath; $PROC_aT_Files)
		For ($PROC_vL_Count; 1; Size of array:C274($PROC_aT_Files))
			DELETE DOCUMENT:C159($folder_platformPath+$PROC_aT_Files{$PROC_vL_Count})
		End for 
		//
		//Now that there are only folders left, let's recursively delete them and all thier po' li'l chillen...
		FOLDER LIST:C473($folder_platformPath; $PROC_aT_Folders)
		For ($PROC_vL_Count; 1; Size of array:C274($PROC_aT_Folders))
			Folder_Delete($folder_platformPath+$PROC_aT_Folders{$PROC_vL_Count}+Folder separator:K24:12)
		End for 
